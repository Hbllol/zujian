(function(){
    window.Slider = function Slider(argsJSON){
        ...
        this.btns=this._findClassName("btns")[0];
    ...
    Slider.prototype._shangyizhang = function(){
        if (this.timer) {
            return;
        }
        // 备份开始张位置
    ...
    Slider.prototype._animate = function(arr,time){
        ...
        this.timer = setInterval(function(){
            ...
            for (var i = 0; i < arr.length; i++){
                // 设置当前图片的移动
                arr[i].obj.style.top = self.tween(currentFrame,arr[i].start,arr[i].target - arr[i].start,maxFame) + "px";
            }
        },this.interval)
    }
    ...
    Slider.prototype._bindEvent=function(){
    	...
    	this.carousel.onmouseover = function () {
            ...
            self.btns.style.display="block"
        }
    	//鼠标离开 ，调用动画 继续运行
    	
    	this.carousel.onmouseout=function(){
    		self._autoplay();
            self.btns.style.display=""
